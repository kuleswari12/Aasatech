        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {sections.map((sec, index) => (
              <div
                key={index}
                className="bg-blue-50 rounded-lg shadow-md p-6 flex flex-col items-center hover:shadow-lg transition-shadow duration-300"
              >
                <h3 className="text-xl font-medium">{sec.title}</h3>
                <button className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded mt-4 w-full sm:w-auto">
                  {sec.button}
                </button>
                <img
                  src={sec.image}
